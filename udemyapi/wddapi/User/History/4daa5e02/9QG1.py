...
async def get_device(user:user_dependency, db:db_dependency, device_id: int):
    ...
    device_ls = db.query(Device).filter(Device.device_id == device_id and Device.owner_id == user.get('user_id')).order_by(Device.load_dt.desc()).first()

    if device_ls is not None:
        return device_ls
    else:
        raise HTTPException(status_code=404, detail=f"Device not found with the device_id {device_id} for user {user.get('username')}")
