...
async def create_token_for_login(form_data: Annotated[OAuth2PasswordRequestForm, Depends()], db:db_dependency):
    ...
    if user:
        token = create_access_token(user.username, user.user_id, user.role_id, timedelta(minutes=32))
...
def check_credentials(username: str, password: str, db: db_dependency):
    ...
    # print values below to check/test them
...
def create_access_token(username:str, user_id: int, role: str, expires_delta: timedelta):
    encode = {'sub': username, 'id': user_id, 'role': role} #payuload contains basic details do not store confidentials info
