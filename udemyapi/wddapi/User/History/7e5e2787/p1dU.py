...
@router.delete("/delete/{device_id}", status_code=status.HTTP_200_OK)
async def read_all(user:user_dependency, db:db_dependency, device_id: int):
    """
    return all the devices in the system with Admin access/role
    """
    if user is None:
    ...
    if user_str_role.name == "admin":
        ...
        if device is not None:
            db.query(Device).filter(Device.device_id == device_id).first().delete()
            db.commit()
        else:
            return 'No Device in the system'
    else:
